...
After the ESP32 board is initially flashed and has joined a LoRaWAN network, the firmware can update itself by FOTA. This process is kicked off by sending a remote control command (see below) via LoRaWAN to the board. The board then tries to connect via WIFI to a cloud service (JFrog Bintray), checks for update, and if available downloads the binary and reboots with it. If something goes wrong during this process, the board reboots back to the current version. Prerequisites for FOTA are: 1. You own a Bintray repository,2.create an Package named identically your halfile (without suffix)  3. you pushed the update binary to the Bintray repository, 4. internet access via encrypted (WPA2) WIFI is present at the board's site, 5. WIFI credentials were set in ota.conf and initially flashed to the board. Step 3 runs automated, just enter the credentials in ota.conf and select `env:ota` in VSCode IDE or `-e ota` in PlatformioCli,  Then press `upload` or `type pio run -t upload -e ota` and lean back watching platformio doing build and upload.
